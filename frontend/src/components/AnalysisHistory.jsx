...
export default function AnalysisHistory({ onOpen, section, disabled = false }) {
  ...
  return <details className="panel p-5 max-w-3xl mx-auto mt-5">
    ...
    {visibleItems.map(item => <div className="flex gap-3 items-start py-2" key={item.analysis_id}>
      <button disabled={disabled} className="text-sm text-left flex-1" onClick={async () => {
        try {
      ...
      <button disabled={disabled} className="btn-secondary" onClick={async () => {
        try {
